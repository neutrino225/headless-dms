...
describe("Document entity", () => {
	describe("create()", () => {
		...
		it("sets createdAt and updatedAt to the current time", () => {
			...
			expect(doc.createdAt.toDate().getTime()).toBeGreaterThanOrEqual(
				before.getTime(),
			);
			expect(doc.createdAt.toDate().getTime()).toBeLessThanOrEqual(
				after.getTime(),
			);
		});

		it("generates a unique id on each call", () => {
			const makeDoc = (slug: string) =>
				Document.create({
					name: "A",
					description: Option.None,
					ownerId: TEST_IDS.user1,
	...
	describe("fromSerialized()", () => {
		...
		it("factory generates realistic timestamps", () => {
			const doc = makeDocument();
			expect(
				new Date(doc.createdAt.toISOString()).getTime(),
	...
	describe("serialize()", () => {
		it("round-trips through serialize → fromSerialized", () => {
			...
			expect(restored.description.safeUnwrap()).toBe(
				original.description.safeUnwrap(),
			);
			expect(restored.ownerId).toBe(original.ownerId);
			expect(restored.slug).toBe(original.slug);
			expect(restored.status).toBe(original.status);
			expect(restored.latestVersionId.safeUnwrap()).toBe(
				original.latestVersionId.safeUnwrap(),
			);
			expect(restored.metadata.safeUnwrap()).toEqual(
				original.metadata.safeUnwrap(),
			);
