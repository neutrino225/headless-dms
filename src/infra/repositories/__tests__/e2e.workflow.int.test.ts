...
describe("E2E Document Workflow", () => {
  ...
  it("should complete full document lifecycle: create → add versions → fetch latest → update", async () => {
    ...
    // Step 6: Update document metadata (with latest version ID updated manually)
    ...
    expect(finalDoc.metadata?.safeUnwrap()).toEqual({ department: "finance", quarter: "Q4", year: 2024 });
    ...
    const slugResult = await documentRepository.fetchBySlug("q4-financial-report");
    expect(slugResult.isOk()).toBe(true);
    expect(slugResult.unwrap().unwrap().id.toString()).toBe(document.id.toString());
  });
