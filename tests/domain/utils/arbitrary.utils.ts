...
const dateTime = {
	...
	past() {
		return {
			arbitrary: () => (fc: any) =>
				fc.integer({ min: 0, max: 730 }).map((daysAgo: number) => {
					const d = new Date();
	...
	recent() {
		return {
			arbitrary: () => (fc: any) =>
				fc.integer({ min: 0, max: 30 }).map((daysAgo: number) => {
					const d = new Date();
					d.setDate(d.getDate() - daysAgo);
					return d.toISOString();
				}),
	...
	future() {
		return {
			arbitrary: () => (fc: any) =>
				fc.integer({ min: 1, max: 30 }).map((daysAhead: number) => {
					const d = new Date();
...
function storageKey() {
	return {
		arbitrary: () => (fc: any) =>
			fc
				.constant(null)
				.map(() => `uploads/${faker.string.uuid()}/${faker.system.fileName()}`),
	};
}
