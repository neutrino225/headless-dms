...
export const TEST_IDS = {
	workspace1: WorkspaceId.fromTrusted("90000000-0000-0000-0000-000000000001"),
	...
	docVersion1: DocumentVersionId.fromTrusted(
...
const TestUserSchema = S.Struct({
	...
	workspaceId: S.String.annotations(refined.uuid()),
	...
	role: S.Literal(UserRole.USER, UserRole.ADMIN).annotations({
		arbitrary: () => (fc: any) =>
			fc.constantFrom(UserRole.USER, UserRole.ADMIN),
	}),
...
export function makeUser(overrides: UserOverrides = {}): User {
	...
	return User.fromSerialized({
		...
		workspaceId: overrides.workspaceId ?? sample.workspaceId,
...
export function makeDocument(overrides: DocumentOverrides = {}): Document {
	...
	return Document.fromSerialized({
		...
		latestVersionId:
			overrides.latestVersionId !== undefined
				? overrides.latestVersionId
				: null,
		metadata:
			overrides.metadata !== undefined ? overrides.metadata : sample.metadata,
...
export function makeDocumentWithStatus(
	status: DocumentStatus,
...
export function makeAccessPolicy(
